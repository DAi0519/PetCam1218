...
async function fetchImageAsBase64(url: string): Promise<string | null> {
  try {
    ...
    console.warn("Failed to fetch accessory image:", error);
    return null;
  }
}
...
async function withRetry<T>(
  ...
  try {
    ...
    if (isOverloaded && retries > 0) {
      ...
      return withRetry(operation, retries - 1, Math.round(baseDelay * 1.5));
    }
...
Only actual photographs of real, living pets should return is_realistic_pet_photo: true.`;
    ...
    const detectResponse = await withRetry(() =>
    ...
    const detectionData = JSON.parse((detectResponse as any).text || "{}");
    const isRealisticPet = detectionData.is_pet && detectionData.is_realistic_pet_photo;

    if (!isRealisticPet) {
      return await generateSimpleChristmasHat(
        ai,
        base64Image,
        mimeType,
        detectionData.subject_description || "the subject"
      );
    }
    ...
    const isQuotaExhausted =
      ...
      String(error?.statusText ?? "").includes("Too Many Requests");

    if (isQuotaExhausted) {
      errorMessage = "抱歉~今日用户量已达上限，请明日再试或联系开发者";
    } else if (errorMessage.includes("overloaded") || error.code === 503) {
      errorMessage = "Server busy (High Traffic). Please try again in a moment.";
    }
...
async function generatePetFashionPortrait(
  ...
  const analysisData = JSON.parse((analysisResponse as any).text || "{}");
